...
char* indexes(const char* path)
{
    ...
    if (directory != NULL)
    {
        while ((dir_item = readdir(directory)) != NULL)
        {
            ...
            {
                full_path = malloc(strlen(file_name) + strlen(path) + 1);
                full_path[0] = '\0';
        ...
        closedir(directory);
        return full_path; // will return NULL if no index file found
    }
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    while ((bytes_read = fread(&buffer, 1, sizeof(buffer), file)) > 0) 
    {
        ...
        loaded_file_location = realloc(loaded_file_location, file_size);
        
        BYTE* first_empty = loaded_file_location + file_size - bytes_read;
        
        memcpy(first_empty, buffer, bytes_read);
    }
...
const char* lookup(const char* path)
{
    ...
    if ((extension_pos = strrchr (path, '.')))
    {
        char* extension = extension_pos + 1;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* next_space = strchr(line + 4, ' ');
    
    if (strncmp("HTTP/1.1", next_space + 1, 8) != 0)
    {
        error(505);
        return false;
    }
    ...
    size_t abs_path_len;
    char* question_mark;
    if ((question_mark = strchr(line + 4, '?')) != NULL)
    {
        abs_path_len = question_mark - (line + 4);

        size_t query_len = next_space - question_mark - 1;
